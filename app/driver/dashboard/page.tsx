...
export default function DriverDashboardPage({ user, logout }: any) {
  ...
  const { historySchedule, fetchDriverTripHistory: fetchPersonnelTripHistory } =
    usePersonnelSchedule();

  const { fetchDriverTripHistory: fetchAssignmentHistory, driverHistory } =
  ...
  const upcomingCount = useMemo(() => {
    if (!historySchedule) return 0;
    const today = getTodayDateKey();

    return historySchedule.filter((item: any) => {
      return (
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 text-white font-bold text-xl border border-white/10 shadow-inner">
                    {currentUser?.name ? currentUser.name.charAt(0) : "D1"}
                  </div>
