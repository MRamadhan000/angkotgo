...
export default function LocationPage() {
   ...
   * }
   */
  const [routePathMap, setRoutePathMap] =
    useState<Record<string, RoutePath[]>>({});
  ...
  const originSession = useRef(
    crypto.randomUUID(),
  );

  const destinationSession = useRef(
    crypto.randomUUID(),
  );

  const selectingOrigin =
    useRef(false);
  ...
  async function searchLocation(
    ...
    if (!TOKEN) {
      setError(
        "NEXT_PUBLIC_MAPBOX_TOKEN belum dikonfigurasi.",
      );
      return;
    }
  ...
  async function retrieveLocation(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Retrieve error: ${response.status}`,
        );
      }
  ...
  async function searchRoutes() {
    if (!origin || !destination) {
      setError(
        "Pilih titik awal dan tujuan terlebih dahulu.",
      );
      return;
    }
    ...
    try {
      ...
      const url =
        `${API_URL}/routes/search?${params.toString()}`;
      ...
      const routes: RouteSearchResult[] =
        Array.isArray(data)
          ? data
          : data?.data ?? [];
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-5 flex justify-end">
          <button
            ...
            className="rounded-xl bg-black px-6 py-3 text-sm font-semibold text-white transition hover:bg-black/80 disabled:cursor-not-allowed disabled:bg-black/20"
          >
            {loading === "route"
              ? "Mencari rute..."
              : "Cari Rute"}
    ...
    <div className="rounded-2xl border bg-white p-5 shadow-sm">
      ...
      <div className="mt-6 border-t pt-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Info
            label="Beeline Total"
            value={`${route.beelineTotal.toFixed(
              2,
            )} m`}
          />
          ...
          <Info
            ...
            value={`${route.totalWalkingDistance.toFixed(
              2,
            )} m`}
          />
          ...
          <div className="rounded-xl border p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              ...
              <Info
                label="Duration"
                value={formatDuration(
                  route.walkingToDestination.duration,
                )}
              />
...
function RoutePathLine({
  ...
  /*
   * Pastikan titik diurutkan berdasarkan
   * sequenceOrder.
   */
  ...
  return (
    <div className="relative">
      ...
      {suggestions.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-[1000] mt-2 overflow-hidden rounded-xl border bg-white shadow-xl">
...
function MapController({
  ...
  useEffect(() => {
    /*
     * Kalau sudah ada RoutePath,
     * fit map berdasarkan semua titik
     * route.
     */
