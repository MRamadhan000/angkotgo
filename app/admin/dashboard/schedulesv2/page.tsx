            ...
            {groupedSchedules.map((group, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6">
                    <h5 className="text-xs font-bold tracking-wider text-slate-400 uppercase mb-4 flex items-center gap-1.5">
                      <Navigation size={12} />
                      Estimasi Waktu Tiba di Halte
                    </h5>

                    <div className="md:overflow-x-auto md:pb-1 md:[scrollbar-width:thin]">
                      <div className="flex flex-col md:flex-row md:items-start gap-0 md:min-w-max md:pr-4">
                        {group.stops.map((stop, i, arr) => {
                          ...
                          return (
                            <Fragment key={stop.stopName + stop.stopOrder}>
                              ...
                              <div className="flex md:flex-col items-start md:items-center gap-3 md:gap-2 md:w-40 md:shrink-0 md:text-center">
                                <div
                                  className={`relative z-10 w-7 h-7 mt-0.5 md:mt-0 shrink-0 rounded-full border-2 flex items-center justify-center text-[10px] font-bold shadow-sm ${dotClass}`}
                                >
                                  {stop.stopOrder}
                                </div>
                                <div className="min-w-0 flex-1 md:flex-none">
                                  <p
                                    className="text-sm font-semibold text-slate-800 md:leading-tight"
                                    title={stop.stopName}
                                  >
                                    {stop.stopName}
                                  </p>
                                    ...
                                    {stop.arrivals.map((arrival) => {
                                      ...
                                      return (
                                        <span
                                          ...
                                          className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-md border ${chipClass}`}
                                        >
                              ...
                              {!isLast && (
                                <div
                                  className="w-0.5 h-6 ml-[15px] border-l-2 border-dashed border-slate-300
                                             md:w-auto md:h-0 md:ml-0 md:mt-[13px] md:flex-1 md:min-w-6 md:self-start
