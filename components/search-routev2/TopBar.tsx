  ...
  return (
    <div className="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white/95 px-3.5 py-2.5 shadow-md shadow-blue-900/5 backdrop-blur-md transition-all">
      ...
      {isAuthenticated && user ? (
        ...
        <div className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-100/80 px-2.5 py-1 text-xs font-medium text-slate-600 shadow-xs sm:px-3">
          <FiUser className="shrink-0 text-xs text-slate-500" aria-hidden="true" />
          <span className="text-[11px] sm:text-xs">Guest</span>
        </div>
      )}
    </div>
  );
