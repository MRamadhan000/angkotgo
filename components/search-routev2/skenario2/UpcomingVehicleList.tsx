...
import type { SuccessfulBooking } from "@/app/getv2/hooks/useBookingState";
...
export default function UpcomingVehicleList({
  ...
  const isPaidMode = successfulBookings.length > 0;
  const displayVehicles = isPaidMode
    ? successfulBookings.map((booking) => booking.vehicle)
    : vehicles;
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-bold text-slate-900 sm:text-base">
              {isPaidMode ? "Booking Saya" : "Pilih Angkot"}
            </h2>
            <span className="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-bold text-[#003d9b] ring-1 ring-blue-600/20">
              {displayVehicles.length} {isPaidMode ? "dibooking" : "tersedia"}
            </span>
          </div>

          <p className="mt-0.5 text-[11px] text-slate-500">
            {isPaidMode
              ? "Pembayaran berhasil. Konfirmasi perjalanan Anda."
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-4 pb-5">
          {displayVehicles.map((vehicle) => (
            <UpcomingVehicleCard
              ...
              onBook={onBook}
              isSelected={
                !isPaidMode && selectedVehicleId === vehicle.assignmentId
              }
              isBookingEnabled={hasBoarded}
              isPaidBooking={isPaidMode}
              isFeedbackSubmitted={feedbackSubmittedIds.includes(
