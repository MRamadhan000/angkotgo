...
export function PaymentEntryModal({
  ...
  const tarifPelajar = (tarifResponse?.data ?? []).find(
    (tarif) => tarif.name.trim().toUpperCase() === "PELAJAR",
  )?.nominal;
  const tarifUmum = (tarifResponse?.data ?? []).find(
    (tarif) => tarif.name.trim().toUpperCase() === "UMUM",
  )?.nominal;
  const quickAmounts = [
    ...(tarifPelajar ? [{ label: "Pelajar", value: tarifPelajar }] : []),
    ...(tarifUmum ? [{ label: "Umum", value: tarifUmum }] : []),
  ];
  ...
  const initiateSelectAmount = (amount: number) => {
    setConfirmData({
      amount,
      type: activeTab === "CASH" ? PaymentType.CASH : PaymentType.QRIS,
    });
  };
                ...
                <>
                  <div className="grid grid-cols-2 gap-3">
                    {quickAmounts.map((amt) => (
                      ...
                      >
                        {amt.label}: Rp {amt.value.toLocaleString("id-ID")}
                      </button>
                    ))}
                  </div>
                  {isTarifLoading && (
                ...
                <form onSubmit={handleCustomSubmit} className="space-y-4 mt-2">
                  <div className="relative">
                    <input
                      ...
                      value={customAmount}
                      onChange={(e) =>
                        setCustomAmount(formatRupiahInput(e.target.value))
                      }
            ...
            <div className="space-y-5">
                ...
                <div className="w-48 h-48 bg-white p-3 rounded-2xl shadow-sm border border-emerald-200">
                  <div className="w-full h-full bg-[radial-gradient(#10b981_3px,transparent_3px)] [background-size:12px_12px] opacity-80 rounded-xl relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <FaQrcode
                        size={42}
                        className="text-emerald-700 p-2 bg-white rounded-lg shadow-sm"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-emerald-50 p-4 rounded-2xl flex flex-col gap-2">
                <p className="text-center text-xs font-semibold text-emerald-700">
                  <FaCheckCircle className="inline mr-1 text-emerald-600" />{" "}
                  (SIMULASI MASUK SYSTEM)
                </p>
                <div className="grid grid-cols-2 gap-2 mt-1">
                  {quickAmounts.map((amt) => (
                    <button
                      key={amt.label}
                      onClick={() => initiateSelectAmount(amt.value)}
                      className="bg-emerald-600 text-white font-bold text-xs p-3 rounded-xl hover:bg-emerald-700"
                    >
                      {amt.label}: Rp {amt.value.toLocaleString("id-ID")}
                    </button>
                  ))}
        ...
        {confirmData && (
          <div className="absolute inset-0 bg-white/95 backdrop-blur-md z-20 flex flex-col items-center justify-center p-6 text-center animate-in min-h-[400px]">
            ...
            <p className="text-slate-600 mb-6 px-4">
              Apakah Anda yakin memasukkan pembayaran senilai
              <br />
              <span className="font-bold text-lg text-slate-900 block mt-2">
                Rp {confirmData.amount.toLocaleString("id-ID")} (
                {confirmData.type})?
              </span>
            </p>
